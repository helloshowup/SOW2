...
# Runner --> DB    : create agent\_runs entry

# Runner --> Redis : cache intermediate data

# Worker    --> Runner : execute tasks from Redis

# Worker    --> DB    : update agent\_runs status + result
...
# 2\. \*\*API runner\*\* enqueues a task in Redis and inserts a new `agent\_runs` row (`pending`).

# 3\. \*\*Worker\*\* fetches the task, runs the agent logic, then updates `agent\_runs` with `status`, `completed\_at`, and the `result` JSON.
